...
@section('container')
    <h2 class="mb-3">Edit Room {{ $room->room_name }}</h2>
    ...
    <div class="col-md-8 mb-5 p-0">
        <form action="{{ route(config("data.route.admin.rooms.update"), $room->id) }}" method="post" enctype="multipart/form-data">
			...
			<div class="mb-3">
				...
				<input type="text" name="room_name" class="form-control @error('room_name') is-invalid @enderror" id="room_name" value="{{ old("room_name", $room->room_name) }}" required autofocus>
				@error('room_name')
